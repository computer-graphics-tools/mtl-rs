...
> [Apple Documentation](https://developer.apple.com/documentation/metal/selecting-device-objects-for-compute-processing?language=objc)
...
```bash
cargo run --example selecting_device_objects_for_compute_processing
```

## Shader Files

- `Renderer/AAPLShaders.metal`
- `Simulation/AAPLKernels.metal`

## Resources

- `Documentation/SingleDeviceExecution.png`
- `Documentation/MultiDeviceBuffer.png`
- `Documentation/SingleDeviceBuffer.png`
- `Documentation/MultiDeviceExecution.png`
...
This sample implements many techniques described in [Selecting device objects for graphics rendering](https://developer.apple.com/documentation/metal/selecting-device-objects-for-graphics-rendering?language=objc). For information about handling external GPU notifications, see the following sections from that sample:
...
When the sample runs on multiple devices, the view controller assigns the simulation work to one device and the renderer work to another. The simulation executes repeatedly in a loop on a separate simulation thread. For each iteration, it updates the positions of the N-body particles and blits this data to a new [`MTLBuffer`](https://developer.apple.com/documentation/metal/mtlbuffer?language=objc) backed by system memory. The sample passes this system memory backing to the view controller, which then passes it along to the renderer. The renderer executes repeatedly in a loop on the main thread. For each iteration, it creates a new [`MTLBuffer`](https://developer.apple.com/documentation/metal/mtlbuffer?language=objc), backed by the same system memory populated by the simulation thread, and renders the N-body particles based on the latest available position data.

> **Note:** An [`MTLBuffer`](https://developer.apple.com/documentation/metal/mtlbuffer?language=objc) can’t be directly transferred between different devices; its data needs to be transferred via system memory.
...
The sample calls the `vm_allocate` function to allocate a page-aligned buffer, `updateAddress`, backed by system memory. The sample then calls the [`newBufferWithBytesNoCopy:length:options:deallocator:`](https://developer.apple.com/documentation/metal/mtldevice/makebuffer(bytesnocopy:length:options:deallocator:)?language=objc) method to create a new [`MTLBuffer`](https://developer.apple.com/documentation/metal/mtlbuffer?language=objc), `_updateBuffer`, backed by the same system memory used for the previous buffer.
