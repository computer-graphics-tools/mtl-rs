...
Layer Selection is supported on all macOS GPUs but only iOS GPUs which support the [`MTLFeatureSet_iOS_GPUFamily5_v1`](https://developer.apple.com/documentation/metal/mtlfeatureset/ios_gpufamily5_v1?language=objc) feature set.

You check the GPU that you choose at runtime supports indirect command buffers (ICBs) by calling an [`MTLDevice`](https://developer.apple.com/documentation/metal/mtldevice?language=objc) instance’s [`supportsFeatureSet:`](https://developer.apple.com/documentation/metal/mtldevice/supportsfeatureset(_:)?language=objc) method.
