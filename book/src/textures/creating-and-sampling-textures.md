...
> [Apple Documentation](https://developer.apple.com/documentation/metal/creating-and-sampling-textures?language=objc)
...
```bash
cargo run --example creating_and_sampling_textures
```
...
- `Documentation/BGRA8UnormBitLayout.png`
- `Documentation/MetalTextureCoordinates.png`
- `Renderer/Image.tga`
...
The Metal framework doesn’t provide an API to directly load image data from a file to a texture. Metal itself only allocates texture resources and provides methods that copy data to and from the texture. Metal apps rely on custom code or other frameworks, like MetalKit, Image I/O, UIKit, or AppKit, to handle image files. For example, you can use [`MTKTextureLoader`](https://developer.apple.com/documentation/MetalKit/MTKTextureLoader?language=objc) to perform simple texture loading. This sample shows how to write a custom texture loader.
...
Use an [`MTLTextureDescriptor`](https://developer.apple.com/documentation/metal/mtltexturedescriptor?language=objc) object to configure properties like texture dimensions and pixel format for an [`MTLTexture`](https://developer.apple.com/documentation/metal/mtltexture?language=objc) object. Then call the [`newTextureWithDescriptor:`](https://developer.apple.com/documentation/metal/mtldevice/maketexture(descriptor:)?language=objc) method to create a texture.
...
Call the [`replaceRegion:mipmapLevel:withBytes:bytesPerRow:`](https://developer.apple.com/documentation/metal/mtltexture/replace(region:mipmaplevel:withbytes:bytesperrow:)?language=objc) method on the texture to copy pixel data from the `AAPLImage` object into the texture.
