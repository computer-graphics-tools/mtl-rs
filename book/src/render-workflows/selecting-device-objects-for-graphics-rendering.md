...
The sample calls the [`MTLCopyAllDevicesWithObserver`](https://developer.apple.com/documentation/metal/mtlcopyalldeviceswithobserver?language=objc) function to obtain all Metal devices available to the system. This method allows the sample to supply an [`MTLDeviceNotificationHandler`](https://developer.apple.com/documentation/metal/mtldevicenotificationhandler?language=objc) block that’s executed when an external GPU is added or removed from the system. This handler provides two arguments:
